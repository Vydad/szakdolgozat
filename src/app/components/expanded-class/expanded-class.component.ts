...
export interface Trait {
  lvl: number;          // Szint (pl. 1, 2, 3)
  title: string;        // Képesség neve (pl. "Dühöngés")
  description: string;  // Képesség leírása
  uses: string;         // Melyik karakterre/klasztra vonatkozik (pl. "Barbár")
}

@Component({
  selector: 'app-expanded-class',
  standalone: true,
  imports: [CommonModule,FormsModule],
  templateUrl: './expanded-class.component.html',
  styleUrls: ['./expanded-class.component.scss']
})
export class ExpandedClassComponent implements OnInit {
  ...
  expandedClassId: string | null = null; // A kiválasztott kaszt azonosítója, hogy azt bővítsük
  ...
  ngOnInit() {
    ...
    this.dataService.getClass().subscribe(data => {
  ...
  groupedTraits(id: string): { level: number, traits: Trait[] }[] {
  ...
  kivalaszt(id:string,armor:string[],weapon:string[],save:string[],mage:boolean,spell_mod:string){
    this.classId=id;
    this.createService.setClass(id);
    this.createService.setHp(this.hp);
    this.createService.setArmor(armor);
    this.createService.setSave(save);
    this.createService.setWeapon(weapon);
    this.createService.setMage(mage,spell_mod);
  }

  updatelvl(event: Event) {
    const input = event.target as HTMLInputElement;
    const value = Number(input.value);
    this.lvl = value;
    this.createService.setLvl(value);
  }
